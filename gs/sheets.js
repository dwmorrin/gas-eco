...
var index = {
  ...
  forms: {
    ...
    CLOSED_NAME     : 'Archive', // TODO update the sheet name on next release
    ID              : 0,
    START_TIME      : 1,
    END_TIME        : 2,
    LOCATION        : 3,
...
/* exported getClosedFormsGAS_ */
function getClosedFormsGAS_(dateRangeJSON) {
  ...
  var sheet = SpreadsheetApp.openById(index.forms.SHEET_ID)
    .getSheetByName(index.forms.CLOSED_NAME);
  var data = sheet.getDataRange().getValues(),
      forms = [];
  data.shift();
...
function writeFormToSheetGAS_(form, close) {
  var ss = SpreadsheetApp.openById(index.forms.SHEET_ID);
  ...
  if (close) {
    var closedForms = ss.getSheetByName(index.forms.CLOSED_NAME);
    closedForms.appendRow(values);
