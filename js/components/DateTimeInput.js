import { createElement, select, option } from "../HTML";
import { getFormattedDateTime } from "../DateUtils";
import { range } from "../Utility";

export default function DateTimeInput({ name, value, onChange }) {
  ...
  const inputs = [
    ...
    select({
      name: `${name}Hour`,
      children: range(12, 1).map((n) => option(n)),
      value: Number(hour), // to remove zero pad
    }),
    document.createTextNode(":"),
    select({
      name: `${name}Minute`,
      children: range(12, 0, 5).map((n) =>
